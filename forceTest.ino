...
int processClawPosition(int motorPosition){
  if (motorPosition < 57)motorPosition = 57;
  return motorPosition;
}
bool elevatorDoor(){
...
void loop() {
  ...
  clawMotor.write(57);
  for (int i=57; i<165; i++){
    if (elevatorDoor() == true){
      Serial.println("Elevator danger!");
      delay(5000);
      break;
    }
    Serial.println(analogRead(A0));
  ...
  if (client) {
    ...
    while (client.connected()) {
      ...
      if (command == "LED ON") { 
        ...
        if (sliderMode == true){
          int angle = (command.substring(7, command.length()+1)).toInt();
          clawMotor.write(processClawPosition(angle));
